...
prepare_apt() {
    ...
    run_root apt-get install -y --no-install-recommends \
        ...
        rustc rustfmt xz-utils zstd

    log "Installing architecture-specific libs..."
    run_root apt-get install -y --no-install-recommends \
        "python3:${BUILD_ARCH}" \
        ...
        "libnetfilter-log-dev:${HOST_ARCH}" "libnspr4-dev:${HOST_ARCH}" \
        ...
        "libsasl2-dev:${HOST_ARCH}" "libsgutils2-dev:${HOST_ARCH}" \
        ...
        "libudev-dev:${HOST_ARCH}" "libusb-1.0-0-dev:${HOST_ARCH}" \
        "libusbredirparser-dev:${HOST_ARCH}" "libxkbcommon-dev:${HOST_ARCH}" \
        "libyang-dev:${HOST_ARCH}" || true   # 允许部分失败
    ...
    run_root apt-get install -y --no-install-recommends \
        librust-proxmox-apt-dev librust-proxmox-docgen-dev || true
